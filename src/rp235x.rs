//! WS2812 PIO Driver for the RP235x
//!
//! This driver implements driving a WS2812 RGB LED strip from
//! a PIO device of the RP235x chip.
//!
...
/// let pins = rp235x_hal::gpio::pin::bank0::Pins::new(...);
///
    ...
    SM: StateMachineIndex,
{
    ...
    pub fn new(
        ...
        let pin = pin.into();
        let (mut sm, _, tx) = rp235x_hal::pio::PIOBuilder::from_installed_program(installed)
            // only use TX FIFO
        ...
        sm.set_pindirs([(pin.id().num, rp235x_hal::pio::PinDir::Output)]);
